...
x=Data[Noutliersxy,9]; y=Data[Noutliersxy,10];
pl=Data[Noutliersxy,2];
v=Data[Noutliersxy,3];
N=length(pl);

#Preparig markets
R=kmeans([x y pl/50]',100);
...
l1=32
l2=52
scatter(markets_sort[l1 .<=markets_sort .<=l2], 1000*v[l1 .<=markets_sort .<=l2], color=:lightrainbow, legend=false)

#################################### Estimation of proxy function and output levels #################################### 
f_po=g_proxy(vbar,plbar,3);

yo=zeros(size(v));
for i in 1:length(yo)
    yo[i]=v[i]/f_po(vbar[markets[i]])
end

#Plotting estimated output per market. 
l1=20
l2=30
scatter(markets_sort[l1 .<=markets_sort .<=l2], yo[l1 .<=markets_sort .<=l2], color=:lightrainbow, legend=false)

# ##################################Below is nothing useful.  IGNORE ##########################

# #Plot the pricing function
